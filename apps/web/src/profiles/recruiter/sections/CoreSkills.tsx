...
export function RecruiterCoreSkills() {
  ...
  const handleCardClick = (skillId: string, rowIndex: number, cardIndex: number) => {
    if (expandedCard === skillId) {
      setExpandedCard(null)
    } else {
      setExpandedCard(skillId)
      
      // Calculate expansion offset for row-aware positioning
  ...
  const programmingItems = programmingRow.cards.map((card, cardIndex) => {
    ...
    return {
      id: `recruiter-programming-${cardIndex}-${slug}`,
      title: card.label,
      targetId: `recruiter-programming-${cardIndex}-${slug}`,
  ...
  const coreAiItems = coreAiRow.cards.map((card, cardIndex) => {
    const slug = card.skill.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '')
    return {
      id: `recruiter-core-ai-${cardIndex}-${slug}`,
      title: card.label,
      targetId: `recruiter-core-ai-${cardIndex}-${slug}`,
      previewSrc: card.previewSrc ?? '',
      chips: card.tags,
      description: card.lines.join(' • '),
    }
  ...
  const systemsItems = systemsRow.cards.map((card, cardIndex) => {
    const slug = card.skill.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '')
    return {
      id: `recruiter-systems-${cardIndex}-${slug}`,
      title: card.label,
      targetId: `recruiter-systems-${cardIndex}-${slug}`,
      previewSrc: card.previewSrc ?? '',
      chips: card.tags,
      description: card.lines.join(' • '),
    }
  ...
  return (
    <section id="recruiter-skills" className="core-skills-section">
      ...
      <div className="skills-row">
        <ContinueWatchingPreview
          ...
          titleId="recruiter-programming-title"
      ...
      <div className="skills-row">
        <ContinueWatchingPreview
          ...
          titleId="recruiter-core-ai-title"
      ...
      <div className="skills-row">
        <ContinueWatchingPreview
          ...
          titleId="recruiter-systems-title"
            ...
            {row.cards.map((card, cardIndex) => {
              ...
              return (
                <div
                  ...
                  onClick={() => handleCardClick(skillId, rowIndex + 1, cardIndex)}
                >
                  <div
                    className="card-image"
                    style={{ background: `linear-gradient(${card.gradient})` }}
                  >
