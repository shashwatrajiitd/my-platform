...
export function AdventurerSkills() {
  ...
  const programmingItems = programmingRow.cards.map((card, cardIndex) => {
    ...
    return {
      id: `adventurer-programming-${cardIndex}-${slug}`,
      title: card.label,
      targetId: `adventurer-programming-${cardIndex}-${slug}`,
  ...
  const coreAiItems = coreAiRow.cards.map((card, cardIndex) => {
    const slug = card.skill.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '')
    return {
      id: `adventurer-core-ai-${cardIndex}-${slug}`,
      title: card.label,
      targetId: `adventurer-core-ai-${cardIndex}-${slug}`,
      previewSrc: card.previewSrc ?? '',
      chips: card.tags,
      description: card.lines.join(' • '),
    }
  ...
  const systemsItems = systemsRow.cards.map((card, cardIndex) => {
    const slug = card.skill.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '')
    return {
      id: `adventurer-systems-${cardIndex}-${slug}`,
      title: card.label,
      targetId: `adventurer-systems-${cardIndex}-${slug}`,
      previewSrc: card.previewSrc ?? '',
      chips: card.tags,
      description: card.lines.join(' • '),
    }
  ...
  return (
    <section id="adventurer-skills" className="core-skills-section">
      ...
      <div className="skills-row">
        <ContinueWatchingPreview
          ...
          titleId="adventurer-programming-title"
      ...
      <div className="skills-row">
        <ContinueWatchingPreview
          ...
          titleId="adventurer-core-ai-title"
      ...
      <div className="skills-row">
        <ContinueWatchingPreview
          ...
          titleId="adventurer-systems-title"
